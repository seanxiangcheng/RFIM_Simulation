...
int main (int argc, char *argv[]) 
{
  /************* System Setup ****************/
    ...
    for (iter = 0; iter < repeat_num; iter++)
    {
        ...
        if(repeat_num>10)
        {
            if( iter%(repeat_num/50)==0 )
            {
                printf("   %.2f%% completed\n", (double)iter/repeat_num*100);
                /**************** Output ***********************/
                //getchar(); //This line keeps the gnuplot window open after the code runs through.
                fpmd=fopen(MED_fn, "w");
                fprintf(fpmd, "2D FM Ising; Repeat_num=%d; L=%d; T=%.2f; s_hi=%.2f; s_Hi=%.2f; MC steps= %-.2e; Final_E=%.2f; Final_M=%d;\n",
                        iter+1, age.get_L(), age.get_T(), age.get_s_hi(),age.get_s_Hi(), total_MCs, age.get_E(), age.get_M());

                fprintf(fpmd, "mc_step,mag,energy,domain_len\n");

                for(i=0; i < RECORD_LEN; i++)
                {
                    fprintf(fpmd, "%.0f,%.8f,%.8f,%.8f\n", rand_steps_record[i]/age.get_N(), 
                    (double)mag_steps[i]/(iter+1.)/age.get_N(), energy_steps[i]/(iter+1.)/age.get_N(), domlen_steps[i]/(iter+1.));
                }
                fclose(fpmd);
                age.print_info();
                printf("   Data Export %d is Done!\n", iter/10+1);
            }
    ...
    fpmd=fopen(MED_fn, "w");
    fprintf(fpmd, "2D FM Ising; Repeat_num=%d; L=%d; T=%.2f; s_hi=%.2f; s_Hi=%.2f; MC steps= %-.2e; Final_E=%.2f; Final_M=%d;\n",
            iter, age.get_L(), age.get_T(), age.get_s_hi(),age.get_s_Hi(), total_MCs, age.get_E(), age.get_M());

    fprintf(fpmd, "mc_step,mag,energy,domain_len\n");

    for(i=0; i < RECORD_LEN; i++)
    {
        fprintf(fpmd, "%.2f,%.8f,%.8f,%.8f\n", rand_steps_record[i]/age.get_N(), 
        (double)mag_steps[i]/(iter+1.)/age.get_N(), energy_steps[i]/(iter+1.)/age.get_N(), domlen_steps[i]/(iter+1.));
    }
    fclose(fpmd);
    age.print_info();
